...
try:
    ...
    f = open('output/user_logins_backup.py', 'r', encoding='utf32')
    saved_user_logins = eval(f.read())
    f.close()
try:
    f = open('output/database.py', 'r', encoding='utf32')
    ...
    try:
        ...
        if r['ip'] == leader:
            if leader != myip:
                ...
                if r['data']['cycle_time'] != cycle_time:
            ...
            return {
                '1': x,
                '2': str(encrypt(x, json.dumps(new_stack).encode('utf32')))
            }
        return {'status': 'not leader'}
    except:
        logging.error(f'follower \n {traceback.format_exc()}')
        return {'status': 'error'}
...
def auth_server():
    ...
    if myip == leader:
        ...
        return {
            ...
            'leader': {
                '1': x,
                '2': str(encrypt(x, json.dumps(leader).encode('utf32')))
            }
        }
...
def test():
    ...
    if leader == myip:
        return {
            '1': x,
            '2': str(encrypt(x, json.dumps({'status': 'leader'}).encode('utf32')))
        }
    else:
        return {
            '1': x,
            '2': str(encrypt(x, json.dumps({'status': 'follower', 'leader': leader}).encode('utf32')))
        }
...
def save_database():
    try:
        ...
        f.close()
        f = open('output/database_backup.py', 'w', encoding='utf32')
        f.write(str(database))
        f.close()
    except:
        logging.error(f'save_database \n {traceback.format_exc()}')
...
def save_user_logins():
    try:
        ...
        f.close()
        f = open('output/user_logins_backup.py', 'w', encoding='utf32')
        f.write(str(user_logins))
        f.close()
    except:
        logging.error(f'save_user_logins \n {traceback.format_exc()}')
